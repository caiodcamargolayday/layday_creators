...
const ASSETS = {
  ...
  strip: [
    ...
    "/layday_gilit CREATORS/final section 3.JPG",
    "/layday_gilit CREATORS/final section 4.JPG",
    "/layday_gilit CREATORS/final section 5.JPG",
    "/layday_gilit CREATORS/final section 6.jpg",
    "/layday_gilit CREATORS/final section 7.JPG",
    "/layday_gilit CREATORS/final section 8.JPG",
  ],
  food_strip: [
    "/layday_gilit CREATORS/food 1.JPG",
    "/layday_gilit CREATORS/food 2.JPG",
    "/layday_gilit CREATORS/food 3.JPG",
    "/layday_gilit CREATORS/food 4.JPG",
    "/layday_gilit CREATORS/food 5.JPG",
    "/layday_gilit CREATORS/food 6.JPG",
    "/layday_gilit CREATORS/food 7.JPG",
  ],
  activities_strip: [
    "/layday_gilit CREATORS/dive and surf 1.jpeg",
    "/layday_gilit CREATORS/dive and surf 2.jpeg",
    "/layday_gilit CREATORS/dive and surf 3.jpeg",
    "/layday_gilit CREATORS/dive and surf 4.jpg",
    "/layday_gilit CREATORS/dive and surf 5.jpg",
  ],
};

const CAROUSEL_IMAGES = [
  "/layday_gilit CREATORS/session 1 carrousel 1.jpg",
  "/layday_gilit CREATORS/session 1 carrousel 2.JPG",
  "/layday_gilit CREATORS/session 1 carrousel 3.JPG",
  "/layday_gilit CREATORS/session 1 carrousel 4.JPG",
  "/layday_gilit CREATORS/session 1 carrousel 5.JPG",
  "/layday_gilit CREATORS/session 1 carrousel 6.JPG",
  "/layday_gilit CREATORS/session 1 carrousel 7.JPG",
];

const CAROUSEL_IMAGES_2 = [
  "/layday_gilit CREATORS/session 2 carrousel 1.jpg",
  "/layday_gilit CREATORS/session 2 carrousel 2.jpg",
  "/layday_gilit CREATORS/session 2 carrousel 3.jpg",
  "/layday_gilit CREATORS/session 2 carrousel 4.JPG",
];
      ...
      <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 z-20">
        {CAROUSEL_IMAGES.map((_, i) => (
          <button 
            key={i}
            onClick={() => setIndex(i)}
            className={`h-1.5 rounded-full transition-all duration-300 ${i === index ? 'bg-[#EE5B2B] w-6' : 'bg-white/60 w-1.5 hover:bg-white'}`}
...
function SlideCarousel() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setDirection(1);
      setIndex((prev) => (prev + 1) % CAROUSEL_IMAGES_2.length);
    }, 3500);
    return () => clearInterval(timer);
  }, [index]);
  ...
  const handleDragEnd = (e: any, { offset }: any) => {
    if (offset.x < -50) {
      ...
      setDirection(-1);
      setIndex((prev) => (prev - 1 + CAROUSEL_IMAGES_2.length) % CAROUSEL_IMAGES_2.length);
    }
  };
      ...
      <div className="absolute top-0 bottom-0 right-4 flex flex-col justify-center gap-2 z-20">
        {CAROUSEL_IMAGES_2.map((_, i) => (
          <button 
            ...
            className={`w-1.5 rounded-full transition-all duration-300 shadow-sm ${i === index ? 'bg-[#EE5B2B] h-6' : 'bg-white/80 h-1.5 hover:bg-white'}`}
            aria-label={`Go to image ${i + 1}`}
          />
        ))}
      </div>
            ...
            {current.type === "textarea" && (
              <div className="w-full flex flex-col gap-4">
                <textarea
                  ...
                  onChange={(e) => setAnswers((a) => ({ ...a, [step]: e.target.value }))}
                  rows={4}
                  className="w-full border-2 border-[#004A61]/20 focus:border-[#EE5B2B] bg-transparent p-4 text-base font-medium outline-none placeholder:text-[#004A61]/30 resize-none transition-colors"
                  autoFocus
                />
                <ContinueBtn disabled={!canContinue || submitting} onClick={next} />
              </div>
      ...
      <section className="relative h-[100dvh] w-full overflow-hidden">
        ...
        <div className="relative z-20 h-full flex flex-col items-center justify-center text-center px-4">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-6xl md:text-8xl font-heading text-white tracking-wider md:tracking-widest leading-none drop-shadow-2xl">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 md:py-12">
        ...
        <section className="mb-10 md:mb-14 -mx-4 overflow-hidden">
          <div className="text-center mb-6">
            ...
            <p className="max-w-2xl mx-auto mt-2 text-sm md:text-base font-medium opacity-70 px-4">
              Fuel your creativity with fresh, vibrant island dishes. From tropical breakfast spreads to social poolside snacks, your stay is fully catered so you can focus on the perfect shot.
            </p>
          </div>
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 px-4">
            {ASSETS.food_strip.map((src, i) => (
              <div key={i} className="relative min-w-[75vw] md:min-w-[320px] aspect-[4/5] border-[4px] border-white shadow-md flex-shrink-0 overflow-hidden">
                <Image src={src} alt={`Food ${i + 1}`} fill className="object-cover hover:scale-105 transition-transform duration-500" sizes="(max-width:768px) 80vw, 320px" />
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-10 md:mb-14 -mx-4 overflow-hidden">
          <div className="text-center mb-6">
            <h2 className="text-xl md:text-3xl font-heading tracking-widest uppercase">
              DIVE INTO <span className="text-[#EE5B2B]">ADVENTURE</span>
            </h2>
            <p className="max-w-2xl mx-auto mt-2 text-sm md:text-base font-medium opacity-70 px-4">
              Dive into crystal-clear turquoise waters or catch world-class breaks. Whether you&apos;re swimming with sea turtles or carving Gili T&apos;s famous waves, every moment is content-gold.
            </p>
          </div>
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 px-4">
            {ASSETS.activities_strip.map((src, i) => (
              <div key={i} className="relative min-w-[75vw] md:min-w-[320px] aspect-[4/5] border-[4px] border-white shadow-md flex-shrink-0 overflow-hidden">
                <Image src={src} alt={`Dive & Surf ${i + 1}`} fill className="object-cover hover:scale-105 transition-transform duration-500" sizes="(max-width:768px) 80vw, 320px" />
              </div>
            ))}
          </div>
        </section>
